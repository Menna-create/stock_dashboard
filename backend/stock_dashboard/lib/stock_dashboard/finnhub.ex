defmodule StockDashboard.Finnhub do                                           
  ...
  def subscribe(symbol) when is_binary(symbol), do: GenServer.cast(__MODULE__,
{:subscribe, symbol})                                                         
  def subscribe(symbols) when is_list(symbols), do: Enum.each(symbols,        
&subscribe/1)                                                                 
...
GenServer.cast(__MODULE__, {:unsubscribe, symbol})                            
  ...
  def get_percentage_change(symbol) do                                        
    case get_latest_trade(symbol) do                                          
      {:ok, %{current_price: current, opening_price: opening}} when opening > 
0 ->                                                                          
  ...
  def get_frontend_data do                                                    
    ...
    |> Enum.map(fn {symbol, data} ->                                          
      %{                                                                      
        ...
        change: calculate_percentage_change(data.current_price,               
data.opening_price),                                                          
        volume: data.volume,                                                  
  ...
  def init(_opts) do                                                          
    # Create ETS tables                                                       
    :ets.new(@trades_table, [:named_table, :ordered_set, :public,             
read_concurrency: true])                                                      
    :ets.new(@stock_data_table, [:named_table, :set, :public,                 
read_concurrency: true])                                                      
  ...
  def handle_info({:websockex_connected, client_pid, _ws_state}, state) do    
    ...
    Enum.each(new_state.subscriptions,                                        
&send_subscribe_message(new_state.ws_client, &1))                             
    {:noreply, new_state}                                                     
  end                                                                         
  ...
  def handle_info({:websockex_frame, {:text, msg_json}, _ws_state}, state) do 
    case Jason.decode(msg_json) do                                            
      ...
      {:ok, %{"type" => "subscribe", "symbol" => symbol}} ->                  
        ...
        new_state = %{state | active_subscriptions:                           
MapSet.put(state.active_subscriptions, symbol)}                               
        {:noreply, new_state}                                                 
  ...
  def handle_info(:reconnect, state) do                                       
    Logger.info("♻️ Attempting reconnect (attempt #{state.reconnect_attempts +
1})")                                                                         
  ...
  def handle_cast({:subscribe, symbol}, state) do                             
    new_state = %{state | subscriptions: MapSet.put(state.subscriptions,      
symbol)}                                                                      
  ...
  defp do_connect(state) do                                                   
    if state.ws_client do                                                     
      ...
      Logger.info("Connecting to #{String.replace(ws_url,                     
~r/token=(.{5})(.*)(.{5})/, "token=\\1...\\3")}")                             
                                                                              
      case StockDashboard.Finnhub.WebSocketClient.start_link(ws_url, self())  
do                                                                            
        {:ok, pid} ->                                                         
          {:noreply, %{state | ws_client: pid, reconnect_attempts:            
...
state.reconnect_attempts + 1})                                                
  ...
  defp schedule_reconnect(state) do                                           
    if @max_reconnect_attempts != :infinity && state.reconnect_attempts >=    
@max_reconnect_attempts do                                                    
  ...
  defp calculate_backoff_delay(attempts) do                                   
    base = @reconnect_initial_delay_ms * :math.pow(2, min(attempts, 6)) |>    
round()                                                                       
  ...
  defp send_pong(ws_client) do                                                
    WebSockex.send_frame(ws_client, {:text, Jason.encode!(%{"type" =>         
"pong"})})                                                                    
  rescue                                                                      
    error ->                                                                  
  ...
  defp calculate_percentage_change(current, opening) when is_number(current)  
and is_number(opening) and opening > 0 do                                     
...
defmodule StockDashboard.Finnhub.WebSocketClient do                           
  ...
  def start_link(url, parent_pid) do                                          
    try do                                                                    
      ...
      kind, reason ->                                                         
        Logger.error("WebSockex start_link caught #{kind}:                    
#{inspect(reason)}")                                                          
        {:error, {kind, reason}}                                              
    end                                                                       
  end                                                                         
  ...
  @impl true                                                                  
  def handle_disconnect(%{reason: reason} = conn_state, %{parent: parent} =   
state) do                                                                     
